...
多设备同步使用独立的服务端运行时变量，**不要**使用 `OKI_` 前缀，避免被 Vite 当作前端构建变量暴露出去。

| 变量名 | 必需 | 适用场景 | 说明 |
| ------ | ---- | -------- | ---- |
| `SYNC_DATABASE_URL` | 是 | Vercel / 自管 Node | Postgres 兼容数据库连接串 |
| `SYNC_SESSION_SECRET` | 是 | Vercel / Docker | 同步账号会话签名密钥，必须使用高强度随机字符串 |
| `SYNC_SESSION_COOKIE_NAME` | 否 | Vercel / Docker | 同步登录 Cookie 名称，默认 `ouonnki_sync_session` |
| `SYNC_DB_NAME` | 否 | Docker Compose | `sync-db` 服务数据库名，默认 `ouonnkitv` |
| `SYNC_DB_USER` | 否 | Docker Compose | `sync-db` 服务用户名，默认 `ouonnki` |
| `SYNC_DB_PASSWORD` | 否 | Docker Compose | `sync-db` 服务密码，默认 `ouonnki_sync` |

### 多设备同步初始化

首次启用同步前，需要先准备数据库并执行迁移：

```bash
pnpm sync:migrate
```

如果使用 Docker Compose 内置的 `sync-db` 服务，请改用：

```bash
pnpm docker:migrate
```

迁移会创建同步一期所需的 `users`、`sessions` 和 `user_sync_state` 表。
